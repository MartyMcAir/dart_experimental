...
class MessageBubble extends StatelessWidget {
  ...
  const MessageBubble.first(
      {super.key, required this.userImage, required this.username, required this.message, required this.isMe})
      : isFirstInSequence = true;

  // Create a amessage bubble that continues the sequence.
  const MessageBubble.next({super.key, required this.message, required this.isMe})
  ...
  Widget build(BuildContext context) {
    ...
    // String? imgPublicUrl;
    // if (userImage != null) {
    //   imgPublicUrl = Supabase.instance.client.storage.from('chatimages').getPublicUrl(userImage!);
    // }
    late final String imgPublicUrl;
    if (userImage != null) {
      final cleanedPath = userImage!.replaceFirst('chatimages/', '');
      imgPublicUrl = Supabase.instance.client.storage.from('chatimages').getPublicUrl(cleanedPath);
      //   final imgPublicUrl = await Supabase.instance.client.storage
      // .from('chatimages')``
      // .createSignedUrl(imgPath, 60 * 60); // 1 hour
    }
      ...
      children: [
        if (userImage != null)
          Positioned(
            ...
            right: isMe ? 0 : null,
            child: CircleAvatar(
              backgroundImage: NetworkImage(imgPublicUrl),
