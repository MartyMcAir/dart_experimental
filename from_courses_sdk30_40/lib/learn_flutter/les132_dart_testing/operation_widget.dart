...
import 'service/number_api_service.dart';

class OperationWidget extends StatefulWidget {
  final Operation operation;
  final Calculator calculator;

  const OperationWidget({super.key, required this.operation, required this.calculator});
    ...
    return ListTile(
      ...
      subtitle: Column(
        children: [
          ...
          ElevatedButton(
            key: Key('elevated_button_${widget.operation.descriptionOperation}'),
            child: const Text('Get Number Info'),
            onPressed: () async {
              if (_operationResult.isNotEmpty) {
                var value = await _numberApiService.getNumberFact((double.parse(_operationResult)).round());
